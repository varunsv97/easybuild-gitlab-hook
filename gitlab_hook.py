...
log.streamInfo("GitLab CI Hook module loaded successfully")
...
def start_hook(*args, **kwargs):
    ...
    log.streamInfo("*** START_HOOK CALLED ***")
...
def pre_build_and_install_loop_hook(ecs, *args, **kwargs):
    ...
    try:
        ...
        log.streamInfo("[GitLab CI Hook] GitLab CI pipeline generated. Stopping EasyBuild execution.")
        raise SystemExit(0)
        ...
        try:
            ...
            if isinstance(easyconfig, dict):
                if 'ec' in easyconfig:
                    ...
                    if ec is None:
                        log.streamInfo("Skipping easyconfig %d: no 'ec' object found", i)
                        continue
                    ...
                    ec = easyconfig.get('ec')
                    if ec is None:
                        log.streamInfo("Skipping easyconfig %d: no 'ec' key found", i)
                        continue
                    easyconfig_name = f"{ec.name}-{ec.version}.eb"
                    spec = easyconfig.get('spec', easyconfig_name)
            else:
...
def _generate_gitlab_pipeline():
    ...
    if not PIPELINE_JOBS:
        log.streamInfo("[GitLab CI Hook] No jobs to generate pipeline for")
        return
...
def _create_gitlab_job(job_info, stage_name):
    ...
    if build_option('dry_run'):
        eb_command += ' --dry-run'
    if build_option('cuda_compute_capabilities'):
        eb_command += f' --cuda-compute-capabilities={build_option("cuda_compute_capabilities")}'
...
def end_hook(*args, **kwargs):
    ...
    if build_option('gitlab_ci_generate'):
        log.streamInfo("[GitLab CI Hook] GitLab CI pipeline generation completed")